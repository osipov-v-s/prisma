...
Упаковка выполняется во временном уникальном каталоге внутри `build/`, после
чего готовый установщик переносится в `src/desktop/release/`. Поэтому случайно
оставленный запущенным старый `release/win-unpacked` больше не блокирует сборку.
Прямой вызов `pnpm --filter @prisma/desktop package:win` staging не использует;
для полного релиза запускайте `scripts/build.py`.
