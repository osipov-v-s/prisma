...
def test_majority_group_always_selects_exactly_two_fastest_observations() -> None:
    ...
    assert aggregation.pair_time_ms == pytest.approx(300.0)
...
def test_all_five_majority_observations_still_select_only_two() -> None:
    ...
    assert [item.level_index for item in aggregation.selected_observations] == [5, 4]
    assert aggregation.pair_time_ms == pytest.approx(30.0)
...
def test_validation_total_time_is_sum_of_two_selected_per_pair() -> None:
    ...
    assert all(item.pair_time_ms == pytest.approx(20.0) for item in aggregations)
...
def test_source_v1_remains_distinct_from_experimental_symmetric_candidate() -> None:
    schema = make_schema(("A", "B", "C", "D"), depth=5)
    winners = repeat_tournament(schema.depth, four_type_moderate_tournament())
    responses = make_responses(schema, winners, uniform_times(schema, 10.0))
    ...
    assert source_percent == pytest.approx(
        [10.555203, 28.593709, 31.230305, 29.620783], abs=1e-6
    )
    assert symmetric_percent == pytest.approx(
        [11.227309, 29.590897, 29.590897, 29.590897], abs=1e-6
    )
    assert difference == pytest.approx(
        [0.672106, 0.997188, -1.639408, -0.029886], abs=1e-6
    )
...
def test_iteration_uses_frozen_previous_vector_and_records_raw_values() -> None:
    ...
    assert result.raw_vectors[0] == pytest.approx([2 / 3, 2 / 3, 2 / 3])
...
def test_time_formula_uses_pair_totals_and_confirmed_direction() -> None:
    binary = np.array(
        [[np.nan, 1.0, 0.0], [0.0, np.nan, 1.0], [1.0, 0.0, np.nan]]
    )
    pair_times = np.array(
        [[np.nan, 10.0, 100.0], [10.0, np.nan, 20.0], [100.0, 20.0, np.nan]]
    )
    weighted = build_time_weighted_matrix(binary, pair_times, 130.0)
    assert weighted[0, 1] == pytest.approx(1 - 10 / 130)
    assert weighted[0, 2] == pytest.approx(100 / 130)
    assert weighted[2, 0] == 1.0
    # Selected C's directed advantage over A equals v_AC.
    assert weighted[2, 0] - weighted[0, 2] == pytest.approx(1 - 100 / 130)
...
def test_result_is_json_serializable_and_versioned() -> None:
    ...
    assert '"algorithm_version": "prisma-analytics/0.4.0-prototype"' in encoded
