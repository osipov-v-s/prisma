...
export function ResultsPage({ serviceBaseUrl, account }: Props) {
  ...
  <div className="result-filters"><input placeholder="Пользователь или коллекция" value={query} onChange={(event) => setQuery(event.target.value)} /><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="">Все статусы</option><option value="completed">Завершён</option><option value="in_progress">В процессе</option></select></div>
